...
SCORE_THRESHOLD = 0.48  # Ngưỡng điểm cosine gốc tối thiểu (0.48). Nếu < 0.48 → fallback sang PageIndex
DEFAULT_TOP_K = 5
...
def retrieve(
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        future_dense = executor.submit(semantic_search, query, top_k * 2)
        future_sparse = executor.submit(lexical_search, query, top_k * 2)
        dense_results = future_dense.result() or []
        sparse_results = future_sparse.result() or []

    # Step 2: Gộp thứ hạng bằng RRF Rerank
    if use_reranking:
    ...
    # Step 3: Kiểm tra điểm Cosine gốc dense_results[0]['score']
    best_dense_score = dense_results[0]["score"] if dense_results and len(dense_results) > 0 else 0.0

    # Step 4: Nếu < score_threshold (0.48) -> Fallback sang PageIndex search
    if best_dense_score < score_threshold:
        print(f"  ⚠ Semantic best score ({best_dense_score:.3f}) < threshold ({score_threshold}). Fallback sang PageIndex.")
        try:
            ...
            print(f"  ⚠ PageIndex fallback search failed: {e}")
